...
public class NativeEncryptorPlugin: NSObject, FlutterPlugin {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    let channel = FlutterMethodChannel(
      name: "native_encryptor", binaryMessenger: registrar.messenger)
  ...
  private func handleEncrypt(call: FlutterMethodCall, result: @escaping FlutterResult) {
    guard
      ...
      result(
        FlutterError(
          code: "INVALID_ARGUMENTS", message: "Missing encryption arguments", details: nil))
      return
    }

    do {
      let encryptedValue = try aesEncryptionService.encrypt(
        passphrase: passPhrase, plainText: contentToEncrypt)
      result(encryptedValue)
    } catch let error {
      result(
        FlutterError(code: "ENCRYPTION_ERROR", message: error.localizedDescription, details: nil))
    }
  }
  ...
  private func handleDecrypt(call: FlutterMethodCall, result: @escaping FlutterResult) {
    guard
      ...
      result(
        FlutterError(
          code: "INVALID_ARGUMENTS", message: "Missing decryption arguments", details: nil))
      return
    }
    ...
    do {
      ...
      result(
        FlutterError(code: "DECRYPTION_ERROR", message: error.localizedDescription, details: nil))
    }
  }
}
